...
import { createContainer, createContainerRest, createContainerWebsocket } from './inversify.config';
...
const missingArgs = ['application-environment', 'root'].filter((option) => !options[option]);
...
const container = /^https?:\/\//.test(options.root)
  ...
  : /^wss?:\/\//.test(options.root)
  ? createContainerWebsocket(options.root)
...
console.log(
  'Try typing in some context, e.g. the empty context: "{}" or clear all your contexts with "clear"'
);
